...
Clazz_load(["JS.Symmetry", "JU.P3", "$.SB"], "J.adapter.smarter.XtalSymmetry", ["java.util.Hashtable", "JU.A4", "$.BS", "$.Lst", "$.M3", "$.M4", "$.P3i", "$.PT", "$.Quat", "$.V3", "J.adapter.smarter.Atom", "JS.SpaceGroup", "$.SymmetryOperation", "$.UnitCell", "JU.BSUtil", "$.Logger", "$.SimpleUnitCell"], function(){
...
function(){
this.getSymmetry().magneticMomentsToFractional(this.asc, this.acr.unitCellParams);
});
...
JS.SymmetryOperation.getRotTransArrayAndXYZ(null, xyz, rotTransMatrix, false, true, false, false, null);
}, "~S,~A");
...
this.spinFramePp = this.spinFrameXyz = null;
}return uvw;
}, "~S,~S,~S");
...
var spinABC = this.setSpinFrameMatrices(unitCellParams);
a = spinABC[1].length();
b = spinABC[2].length();
c = spinABC[3].length();
if (this.perpUVW != null) {
var inv = JU.M4.newM4(this.spinFramePp).invert();
var rot =  new JU.M3();
inv.getRotationScale(rot);
rot.rotate(this.perpUVW);
this.toCartesian(this.perpUVW, true);
this.spinFrameRotationMatrix = JU.Quat.newVA(this.perpUVW, 0).getMatrix();
} else {
}this.spinFrameXyz =  new JU.M4();
var v =  new JU.P3();
for (var i = 4; --i > 0; ) {
v.setT(spinABC[i]);
if (this.spinFrameRotationMatrix != null) this.spinFrameRotationMatrix.rotate(v);
this.spinFrameXyz.setColumn4(i - 1, v.x, v.y, v.z, 0);
}
System.out.println("XtalSymmetry spinFramePp=" + this.spinFramePp + "\nspinFrameXyz=" + this.spinFrameXyz);
...
System.out.println("XtalSymmetry.transformUVW using frame " + this.spinFrame);
...
var spinABC = JS.UnitCell.getMatrixAndUnitCell(this.unitCell, this.spinFrame, this.spinFramePp);
var spinFrameAxisType = "axisxyz";
var strAxis = this.getSpinExt(this.spinFrameExt, "axisxyz");
if (strAxis == null) strAxis = this.getSpinExt(this.spinFrameExt, "axisxyz");
if (strAxis == null) strAxis = this.getSpinExt(this.spinFrameExt, spinFrameAxisType = "perpuvw");
if (strAxis != null) {
...
v = v.clone();
if (this.spinFrameXyz != null) {
this.spinFrameXyz.rotate(v);
...
JS.UnitCell.getMatrixAndUnitCell((transform.indexOf("*") >= 0 ? this.unitCell : null), transform, trm);
return trm;
}, "~S,JU.M4");
...
;//5.0.1-v7 Mon May 12 16:34:37 CDT 2025
Clazz_declarePackage("JS");
...
xyz = xyz.substring(0, pt);
var v =  Clazz_newFloatArray (16, 0);
JS.SymmetryOperation.getRotTransArrayAndXYZ(null, suvw, v, true, false, false, true, "uvw");
this.spinU = JU.M4.newA16(v);
this.timeReversal = Clazz_floatToInt(this.spinU.determinant3());
allowScaling = false;
}var strOut = JS.SymmetryOperation.getRotTransArrayAndXYZ(this, xyz, this.linearRotTrans, allowScaling, halfOrLess, true, false, null);
...
xyz = JS.SymmetryOperation.getRotTransArrayAndXYZ(null, xyz, v, false, halfOrLess, true, false, null);
...
function(xyz){
try {
return JS.SymmetryOperation.getRotTransArrayAndXYZ(null, xyz, null, false, true, true, false, null);
} catch (e) {
if (Clazz_exceptionOf(e, Exception)){
return null;
} else {
...
function(op, xyz, linearRotTrans, allowScaling, halfOrLess, retString, rationalFractions, labels){
...
return null;
}, "JS.SymmetryOperation,~S,~A,~B,~B,~B,~B,~S");
...
JS.SymmetryOperation.getRotTransArrayAndXYZ(null, xyz, a, true, false, false, false, labels);
...
;//5.0.1-v7 Mon May 12 21:36:36 CDT 2025
Clazz_declarePackage("JS");
...
var oabc = JS.UnitCell.getMatrixAndUnitCell(uc, "a,b,c", mSpinPp);
JS.UnitCell.getMatrixAndUnitCell(null, sdef.$replace('*', ' '), mSpinPp);
...
var s = "rotation_axis=" + (aa.x == Math.round(aa.x) ? "" + Math.round(aa.x) : "" + aa.x) + "," + (aa.y == Math.round(aa.y) ? "" + Math.round(aa.y) : "" + aa.y) + "," + (aa.z == Math.round(aa.z) ? "" + Math.round(aa.z) : "" + aa.z);
if (ptAxis < 0) {
...
;//5.0.1-v7 Mon May 19 16:37:14 CDT 2025
Clazz_declarePackage("JS");
