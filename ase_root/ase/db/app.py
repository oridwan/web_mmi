...
def new_app(projects):
    ...
    def update(sid: int, what: str, x: str):
        ...
        * page: show page x
        ...
        # Electride projects should start with a fixed default column set/order.
        if session.project_name in {'electride_binary', 'electride_ternary'}:
            if what == 'query' and x == '0':
                session.columns = electride_default_columns.copy()
            elif 'composition' in session.columns and 'formula' in project.key_descriptions:
                # Keep legacy sessions from showing reduced composition by default.
                session.columns.remove('composition')
                if 'formula' not in session.columns:
                    session.columns.insert(1, 'formula')

        table_keys = list(project.key_descriptions)
        if session.project_name in {'electride_binary', 'electride_ternary'}:
            table_keys = list(dict.fromkeys(table_keys + electride_default_columns))
